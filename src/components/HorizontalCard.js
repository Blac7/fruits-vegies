...
const HorizontalCard = ({
    ...
    const showDiscount = () => {
        if(fruit.discount > 0 &&  fruit.discountQuantity > 0) {
            return (
                <span>
                    <strong>{fruit.discount}%</strong> on <strong>{fruit.discountQuantity} kgs</strong>
                </span>
            )
        } else {
    ...
    const showIncQuantity = IncQuantity => {
        return IncQuantity && (
            <div className="input-group mb-3">
                ...
                <input onChange={handleChange(fruit._id)} style={{width:"50px"}} type="number" id="incQuantity" className="form-control w-2" value={count} max="10"/>
            </div>
        )
    }
    ...
    const cardTitle = () => {
        if(fruit.category === 'fruits') {
            return "card-title fru p-3"
        } else {
             return "card-title veg p-3"
        }
    }
